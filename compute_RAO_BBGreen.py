...
from air_cushion import wave_pumping_excitation_sesx, air_cushion_area, read_fan_characteristics, \
    interpolate_fan_characteristics
...
h_b = 0.64  # [m]  Height of the air cushion  # TODO: make sure this value is correct
...
for i in range(n_frequencies):
    for j in range(6):
        f_ex[j, i] = REFORCE[j, i, 0, 0] + 1j * IMFORCE[j, i, 0, 0]

# compute wave pumping
f_7 = wave_pumping_excitation_sesx(x_f, x_s, y_s, x_b, omega_e, beta)

f_ex = np.vstack([f_ex, f_7])  # append the wave pumping as a seventh excitation
...
max_encounter_frequency = 1000  # Hz

encounter_frequency_Hz = omega_e / 2 / np.pi  # compute encounter frequency in Hz

if plot_raos:
    plt.plot(encounter_frequency_Hz, raos_magnitude[:, 2], '-x', label='python')
    ...
    plt.plot(encounter_frequency_Hz, np.multiply(np.power(omega_e, 2), raos_magnitude[:, 2]), '-x', label='python')
    plt.xlabel('encounter frequency [Hz]')
    plt.ylabel('$|\\omega^2\\eta_{3}|/\\zeta_a[s^{-2}]$')
    plt.title('RAO for acceleration in heave')
    plt.xlim([0, max_encounter_frequency])
    plt.ylim([0, np.max(np.multiply(np.power(omega_e, 2), raos_magnitude[:, 2])[encounter_frequency_Hz < max_encounter_frequency])])
    plt.show()
