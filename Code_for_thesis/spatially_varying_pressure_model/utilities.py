...
def solve_mean_value_relation(n_B_AP, n_B_FP, L, b, x_cp, A_c, p_0, k_2_AP, k_2_FP, k_3, h_s_AP, h_s_FP, C_33, C_55,
                              C_35, C_53, rho_a=1.225):
    """
...
def rms_leakage(x, omega_0s, eta_3_amps, eta_5_amps, H_s, T_p, zeta_a, g=9.81):
    ...
    for i in range(len(k)):
        if zeta_a[i] > 0.0:
            integrand[i] = np.power((np.abs(eta_3_amps[i] - x*eta_5_amps[i] - zeta_a[i]*np.exp(1j*k[i]*x))/zeta_a[i]),
                                    2)*wave_spectrum[i]  # have taking into account change in sign
