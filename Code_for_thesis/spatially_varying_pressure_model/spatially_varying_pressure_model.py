...
from utilities import K_1, K_2, K_3, K_4, Xi_j, Omega_j, solve_mean_value_relation, A_0_AP, A_0_FP, A_0j, A_3j,\
    A_5j, A_7j, B_0j, B_3j, B_5j, B_7j, r_j, solve_linear_systems_of_eq, N_R, N_B, rms_leakage

"""Implementation of analysis with spatially varying pressure for the SES-X vessel BBGreen. The theory is based on 
    Steen and Faltinsen (1995). 'Cobblestone Oscillations of an SES with Flexible Bag Aft Seal'
"""
...
h_0 = 0.64  # [m]  Height of the air cushion
...
V_c0 = A_c0 * h_0  # [m^3] Compute mean air cushion volume

# Read in fan characteristics
Q, P, rpm_dummy = read_fan_characteristics(
    'C:/Users/mathi/code/repos/SES-X-response/Input files/fan characteristics/fan characteristics.csv', '1800rpm')
...
if veres_formulation == 'high-speed':
    ...
    path_high_speed_theory = \
        'C:/Users/mathi/SIMA Workspaces/Workspace_1/Task/BBGreen_uniform_pressure_model_hs_theory/DWL'
    A_temp, B_temp, C_temp, f_ex_temp, omega_0, U, beta, XMTN, ZMTN, lcg_veres, vcg_veres = \
        read_coefficients_from_veres(path_high_speed_theory)
else:
    # For strip_theory
    path_strip_theory = \
        'C:/Users/mathi/SIMA Workspaces/Workspace_1/Task/BBGreen_uniform_pressure_model_strip_theory/DWL'
    A_temp, B_temp, C_temp, f_ex_temp, omega_0, U, beta, XMTN, ZMTN, lcg_veres, vcg_veres = \
        read_coefficients_from_veres(path_strip_theory)
...
lcg_fan = L/2 - x_F_c - x_cp  # [m] Longitudinal fan position (from CG)
...
F_wp = wave_pumping_excitation_sesx(x_f, x_s, y_s, x_b, omega_0, U, beta)  # Compute wave pumping
...
while ((rel_err > epsi) or (counter < 2)) and (counter < max_iter):
    ...
    for j in range(1, j_max + 1):
        ...
        xi_j = Xi_j(j, rho_0, p_0, h_0, b, L, k_2_AP, k_2_FP, a_0_AP, a_0_FP, dQdp_0,
                    lcg_fan + x_cp)  # [-] Relative damping ratio of mode j
        omega_j = Omega_j(j, L)
